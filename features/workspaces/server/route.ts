...
const app = new Hono()
  ...
  .patch(
    ...
    async (c) => {
      ...
      if (!member || member.role !== EnumTypeMember.ADMIN) {
        return c.json({ err: "Unauthorized" }, 401);
      }

      // Lấy document hiện tại để giữ ảnh cũ
      ...
      if (image instanceof File) {
        ...
        uploadedImageUrl = currentWorkspace.image;
      }
  ...
  .post("/:workspaceId/join/:code", sessionMiddleware, async (c) => {
